...
@case("화면", "카톡 원문은 화면에서 차단된다", "2026-08-26 — 조용히 '1건 읽었습니다' 하던 사고")
def _():
    a = I.assess_text("2026년 8월 26일 오전 9:14, 김순자 : 이정희 010-3273-1229\n"
                      "2026년 8월 26일 오전 9:15, 김순자 : 서울 강남구 테헤란로 152")
    return (a["차단"] and "카톡원문" in a["차단사유"],
            f"차단={a['차단']} 사유={a['차단사유']}")


@case("화면", "양식대로 온 주문은 차단되지 않는다", "쉬운 건 화면에서 바로 처리")
def _():
    a = I.assess_text("받는분: 이정희\n연락처: 010-3273-1229\n"
                      "주소: 서울 강남구 테헤란로 152\n수량: 2박스\n보내는분: 김순자")
    return (not a["차단"] and a["권장"] == "단독 판독 가능",
            f"차단={a['차단']} 권장={a['권장']}")


@case("화면", "위험하지만 읽을 수는 있는 입력은 경고만", "가려진 번호는 표를 보여주되 짚어준다")
def _():
    a = I.assess_text("이영희 / 010...4821 / 1박스 / 부산 해운대구 센텀중앙로 55")
    return (not a["차단"] and a["권장"] == "판독팀" and "가려진번호" in a["신호"],
            f"차단={a['차단']} 권장={a['권장']} 신호={list(a['신호'])}")


@case("화면", "차단 입력은 서버도 기록을 거부한다", "화면은 우회될 수 있다")
def _():
    body = (ROOT / "peach-bot/ui/server.py").read_text(encoding="utf-8")
    # add() 안에서 원문을 다시 평가하고 차단하는지
    return ('assess = intake_mod.assess_text(text) if text else None' in body
            and 'if assess and assess["차단"]:' in body,
